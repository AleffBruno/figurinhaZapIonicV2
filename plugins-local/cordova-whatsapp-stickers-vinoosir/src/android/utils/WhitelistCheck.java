...
public class WhitelistCheck {
    ...
    private static boolean isWhitelistedFromProvider(@NonNull Context context, @NonNull String identifier, String whatsappPackageName) {
        ...
        if (isPackageInstalled(whatsappPackageName, packageManager)) {
            ...
            final String stickerAppAuthority = StickerContentProvider.getAuthority(context);
            final Uri queryUri = new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(whatsappProviderAuthority).appendPath(QUERY_PATH).appendQueryParameter(AUTHORITY_QUERY_PARAM, stickerAppAuthority).appendQueryParameter(IDENTIFIER_QUERY_PARAM, identifier).build();
